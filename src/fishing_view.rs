...
fn fish_area_bobber(
    mut commands: Commands,
    mut fish_details: Query<(&mut Fish, &Species, &mut Transform, &mut Visibility), (With<InPond>, With<Collision>, Without<PhysicsObject>, Without<Lure>)>,
    mut bobber: Query<(&Transform, &Tile, Entity, &PhysicsObject, &mut Visibility), (With<Lure>, Without<InPond>, Without<Species>, Without<Fish>)>,
    ...
    time: Res<Time>,
    mut fishes: Query<(Entity, &Fish, &Species, &mut PhysicsObject, &mut Transform, &mut Visibility), (With<Fish>, Without<Lure>)>, //add this in as the fish query, change the position of it at the end 
) {
